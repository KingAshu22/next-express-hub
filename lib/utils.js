...
// Parse zip code ranges like "2003,2005,2010-2015" into array
export function parseZipCodes(input) {
  if (!input) return []
  ...
  parts.forEach((part) => {
    if (part.includes("-")) {
      const [start, end] = part.split("-").map((x) => Number.parseInt(x, 10))
...
// Format zip codes back to grouped format
export function formatZipCodes(codes) {
  if (!codes || codes.length === 0) return ""

  const sorted = codes.map((c) => Number.parseInt(c, 10)).sort((a, b) => a - b)
  const groups = []
  let start = sorted[0]
  let end = sorted[0]
