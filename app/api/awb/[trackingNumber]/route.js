...
export async function GET(req, { params }) {
    ...
    try {
        ...
        return NextResponse.json({ message: "Failed to fetch AWB data.", error: error.message }, { status: 500 });
    }
}
...
export async function PUT(req, { params }) {
    try {
        ...
        return new Response(JSON.stringify({ message: "AWB updated successfully!", updatedAwb }), {
            status: 200,
            headers: { "Content-Type": "application/json" },
        });
    } catch (error) {
        console.error("Error updating AWB data:", error);
        return new Response(JSON.stringify({ message: "Failed to update AWB data.", error: error.message }), {
            status: 500,
            headers: { "Content-Type": "application/json" },
        });
    }
